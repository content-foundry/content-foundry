...
> extends BfEdgeBase<TProps, TMetadata> {
   ...
   */
  static override async querySourceEdgesForNode<
    TProps extends BfEdgeBaseProps,
    TSourceClass extends typeof BfNodeBase,
  >(
    node: BfNodeBase,
    SourceClass: TSourceClass,
  ): Promise<Array<BfEdgeBase<TProps, BfMetadataEdgeBase>>> {
    return Array.from(inMemoryEdges.values()).filter((edge) =>
   ...
   */
  static override async queryTargetEdgesForNode<
    TProps extends BfEdgeBaseProps,
   ...
   */
  override async deleteAndCheckForNetworkDelete(): Promise<void> {
  ...
  override async save(): Promise<this> {
    const key = `${this.sourceId}:${this.targetId}:${this.role}`;
    inMemoryEdges.set(key, this as unknown as BfEdgeInMemory);
    return this;
  }
  ...
  override async delete(): Promise<boolean> {
    const key = `${this.sourceId}:${this.targetId}:${this.role}`;
    return inMemoryEdges.delete(key);
  }
