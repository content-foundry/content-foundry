...
export class DatabaseBackendPostgres implements DatabaseBackend {
  ...
  async putItem(
    ...
    await sql`
      INSERT INTO bfdb (
        ...
        ${s_className || null}, ${sid?.toString() || null}, ${
      t_className || null
    }, ${tid?.toString() || null},
  ...
  > {
    ...
    for (const [key, value] of Object.entries(query)) {
      if (key === "bf_sid" || key === "bf_tid") {
        conditions = sql`${conditions} AND ${sql(key)} = ${value}`;
    ...
    // Ensure we handle the query results properly by asserting the array type
    ...
    return resultArray.map((item) => ({
      ...
      sid: item.bf_sid ? (item.bf_sid as unknown as BfGid) : undefined,
      t_className: item.bf_t_class_name,
      tid: item.bf_tid ? (item.bf_tid as unknown as BfGid) : undefined,
    }));
  }
