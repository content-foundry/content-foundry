...
const TEST_CONTENT_DIR = fromFileUrl(new URL("./test_content", import.meta.url));
...
async function setupTest() {
  ...
  // Preload the collection to avoid async leaks
...
Deno.test("BfContentCollection - findX should throw for non-existent content", async () => {
  ...
  try {
    ...
    try {
      ...
      assert(e instanceof BfErrorNodeNotFound, "Should be a BfErrorNodeNotFound");
    }
  } finally {
    await cleanupTestContent();
  }
...
Deno.test("BfContentCollection - query should return filtered content", async () => {
  ...
  try {
    ...
    assertEquals(allContent.length, 2, "Should only have the two test files we created");
    ...
    assert(titles.includes("Nested Test Post"), "Should include 'Nested Test Post'");

    // Query only featured content
    const featuredContent = await BfContentCollection.query(cv, undefined, {
      featured: true,
    });
    assertEquals(featuredContent.length, 1, "Should have only one featured post");
