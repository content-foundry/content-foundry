...
async function processFolderOrFile(
  ...
  for await (
    ...
    if (shouldIgnore(entry.path, opts.ignorePatterns, opts.ignoreFilesOnly)) continue;
    if (!opts.ignoreGitignore && matchesGitignore(entry.path, gitignorePatterns)) continue;
