...
const loggerGithub = getLogger("github_annotations");
...
async function runCompileWithAnnotations(): Promise<number> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("error: TS")) {
      ...
      if (match) {
        const filePath = match[1].replace("file://", "").replace(Deno.cwd(), "");;
...
async function runLintWithGithubAnnotations(): Promise<number> {
  ...
  //  }
  for (
    const diag of (parsed.diagnostics ?? []) as Array<Record<string, unknown>>
  ) {
